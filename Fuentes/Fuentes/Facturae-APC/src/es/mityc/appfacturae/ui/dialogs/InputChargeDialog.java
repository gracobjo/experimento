...
public class InputChargeDialog extends JDialog {
	...
	private static final long serialVersionUID = 6210247439372667115L;
	...
	public InputChargeDialog (Frame parent, boolean modal, Double total, String version, boolean global) {
	    ...
	    this.codeVersion = FacturaeUtil.getVersionConst(version);
	    this.global = global;
	    init();
	...
	private void init() {
		...
		jButtonOk.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent evt) {
				...
				//20150330:Si no han rellenado el porcentaje de cargo lo rellenamos nosotros porque nos hará falta para aplicarlo a las cuotas de las líneas
			/*	else if (percentTextField.getText() == null || "".equals(percentTextField.getText().trim()) ) {
					// Se establece el porcentaje a descontar calculado a partir del total
					Double doubleObj = DoubleUtil.getDoubleFromPrettyNumber(amountTextField.getText());
					doubleObj = DoubleUtil.roundByVersion(doubleObj, Integer.parseInt(decimalProps.getProperty("charges_total")), version);
					Double porcentaje = (100 * doubleObj) / total ;
					percentTextField.setText(DoubleUtil.roundByVersionFormatted(porcentaje, 
							Integer.parseInt(decimalProps.getProperty("global_charge_rate")), version));
					percentTextField.setBackground(Color.WHITE);
				}*/
	...
	/**
	 * 
	 * item_charge_rate: Invoices-Invoice-Items-Charges-Charge-ChargeRate
	 *
	 */
		...
		public void focusLost(FocusEvent evt){
			...
			if (value != null && !"".equals(value)) {
				...
				try{
					...
					doubleObj = DoubleUtil.roundByVersion(doubleObj, Integer.parseInt(decimalProps.getProperty("item_charge_rate")), version);
...
//					EDT 18/04/2016
//					((JTextField)evt.getSource()).setText(DoubleUtil.formatDecimal(doubleObj, Integer.parseInt(decimalProps.getProperty("item_charge_rate"))));
					((JTextField)evt.getSource()).setText(StringUtil.disableScientificNotation(doubleObj, Integer.parseInt(decimalProps.getProperty("item_charge_rate"))).replace(",", "."));
//					
				}
				...
				amountTextField.setText(DoubleUtil.roundByVersionFormatted(doubleObj, Integer.parseInt(decimalProps.getProperty("item_charge")), version));
	...
	private class AmountValidator extends FocusAdapter {
		public void focusLost(FocusEvent evt){
			...
			if (value != null && !"".equals(value)) {
				...
				try{
					doubleObj = DoubleUtil.getDoubleFromPrettyNumber(value);
					((JTextField)evt.getSource()).setText(DoubleUtil.roundByVersionFormatted(doubleObj, Integer.parseInt(decimalProps.getProperty("item_charge")), version));
				}
				catch(NumberFormatException nfe){
				...
				if (value != null && !"".equals(value))
					rateReaded = DoubleUtil.getDoubleFromPrettyNumber(value);
				else
					return;
				Double amountExpected = (total * rateReaded.doubleValue()) / 100;
				amountExpected = DoubleUtil.roundByVersion(amountExpected, Integer.parseInt(decimalProps.getProperty("item_charge")), version);
				value = ((JTextField)evt.getSource()).getText().trim();
				Double amountReaded = null;
				amountReaded = DoubleUtil.getDoubleFromPrettyNumber(value);
