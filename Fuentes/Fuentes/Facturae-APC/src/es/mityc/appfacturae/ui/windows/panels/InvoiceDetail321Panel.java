/**
 * Copyright 2015 Ministerio de Industria, EnergÃ­a y Turismo
 *
...
public class InvoiceDetail321Panel extends JPanel {
	...
	private void initComponents() {
		...
		((JNumericField) jTextFieldWithPerc).setPrecision(Integer
				.parseInt(decimalProps.getProperty("item_charge_rate")));
		...
		jScrollPaneGlobDisc.getViewport().setBackground(
				Constants.BKG_MAIN_COLOR);
		jTableGlobDisc.setModel(new NoEdiTableModel(new Object[][] {
				...
				Constants.LANG.getString("Amount") + " ("
						+ Constants.LANG.getString("EuroSimbol") + ")" },
				new int[] {}));

		for (int i = 0; i < jTableGlobDisc.getColumnCount(); i++) {
			jTableGlobDisc.getColumnModel().getColumn(i)
		...
		jTableGlobDisc
				...
				.getColumn(0)
				.setCellRenderer(
						new CustomCellRenderer(Constants.SELECTION_COLOR,
								java.awt.Color.white, Constants.FONT_COLOR,
								Constants.FONT_COLOR, Constants.FONT_PLAIN,
		...
		jLabelTotalDisc.setText(Constants.LANG.getString("TotalDiscount")
				+ " (" + Constants.LANG.getString("EuroSimbol") + ")");

		jButtonDiscAdd.setIcon(new ImageIcon(getClass().getResource(
				"/images/button_mini_plus.jpg")));
		...
		jButtonDiscDel.setIcon(new ImageIcon(getClass().getResource(
				"/images/button_mini_minus.jpg")));
			...
			public void focusLost(FocusEvent evt) {
				...
				if (value != null && !"".equals(value.trim())) {
					if (value3 == null || "".equals(value3.trim())) {
						...
						((GenerateInvoice321Window) jTextFieldWithPerc
								.getTopLevelAncestor()).showMessage(
								Constants.LANG.getString("ParameterRequired"),
								Constants.ERROR_MSG_COLOR);
					} else {
						// Amount may be a 2500 max. String
						if (value == null || value.length() > 2500) {
							((JTextField) evt.getSource()).setText("");
							((GenerateInvoice321Window) jTextFieldWithItem
									.getTopLevelAncestor()).showMessage(
									Constants.LANG
											.getString("NOOKParamTooHigh"),
									Constants.ERROR_MSG_COLOR);
						}
					}
		...
		jTextFieldWithPerc.addFocusListener(new FocusAdapter() {
			public void focusLost(FocusEvent evt) {
				...
				if (value2 != null && !"".equals(value2)) {
					...
					try {
						doubleObj = DoubleUtil
								.getDoubleFromPrettyNumber(value2);
						...
						((JTextField) evt.getSource()).setText(DoubleUtil.roundByVersionFormatted(
								doubleObj, Integer.parseInt(decimalProps
										.getProperty("item_charge_rate")),
								version));
					} catch (Exception e) {
						((JTextField) evt.getSource()).setText("");
						((GenerateInvoice321Window) jTextFieldWithPerc
								.getTopLevelAncestor()).showMessage(
								Constants.LANG
										.getString("NOOKParamNumericFormat"),
								Constants.ERROR_MSG_COLOR);
						return;
					}
					...
					if (totalMsg != null && !"".equals(totalMsg)) {
						Double total = DoubleUtil
								.getDoubleFromPrettyNumber(totalMsg);
						...
						// showMessage(Constants.LANG.getString("NOOKParamNegative"),
						// Constants.ERROR_MSG_COLOR);
						// }
						jTextFieldWithAmou.setText(DoubleUtil
								.roundWithPrecisionDoubleFormatted(doubleObj,
										Integer.parseInt(decimalProps
												.getProperty("item_tax_With"))));
						jTextFieldWithAmou.setBackground(Color.white);
						jTextFieldResults[7].setText(DoubleUtil
								.roundWithPrecisionDoubleFormatted(doubleObj,
										Integer.parseInt(decimalProps
												.getProperty("item_tax_With"))));
					} else {
						...
						((GenerateInvoice321Window) jTextFieldWithPerc
								.getTopLevelAncestor()).showMessage(
								Constants.LANG
										.getString("NOOKMessageAmountRequired"),
								Constants.ERROR_MSG_COLOR);
					}
					recalculateTotals();
					if (value == null || "".equals(value.trim())) {
		...
		jTextFieldWithAmou.addFocusListener(new FocusAdapter() {
			public void focusLost(FocusEvent evt) {
				...
				if (value != null && !"".equals(value.trim())
					...
					((GenerateInvoice321Window) jTextFieldWithAmou
							.getTopLevelAncestor()).showMessage(
							Constants.LANG.getString("ParameterRequired"),
							Constants.ERROR_MSG_COLOR);
				} else if (value3 != null && !"".equals(value3.trim())) {
					...
					try {
						doubleObj = DoubleUtil
								.getDoubleFromPrettyNumber(value3);
						((JTextField) evt.getSource()).setText(DoubleUtil
								.roundWithPrecisionDoubleFormatted(
										doubleObj,
										Integer.parseInt(decimalProps
												.getProperty("global_withheld"))));
						jTextFieldResults[7].setText(DoubleUtil
								.roundWithPrecisionDoubleFormatted(
										doubleObj,
										Integer.parseInt(decimalProps
						...
						if (((JTextField) evt.getSource()).getText() != null
								&& !"".equals(((JTextField) evt.getSource())
										.getText().trim())) {
							((JTextField) evt.getSource()).setText("");
							((GenerateInvoice321Window) jTextFieldWithAmou
									.getTopLevelAncestor()).showMessage(
					...
					if (value2 != null && !"".equals(value2.trim())) {
						Double rateReaded = DoubleUtil
								.getDoubleFromPrettyNumber(value2); // %
						String totalMsg = jTextFieldResults[6].getText(); // Total
																			// cost
						Double total = null;
						if (totalMsg != null && !"".equals(totalMsg))
							total = DoubleUtil
									.getDoubleFromPrettyNumber(totalMsg);
						Double amountExpected = (total * rateReaded
								.doubleValue()) / 100; // Amount expected
						amountExpected = DoubleUtil.roundWithPrecisionDouble(
								amountExpected, Integer.parseInt(decimalProps
										.getProperty("global_withheld")));
						Double amountReaded = null;
						amountReaded = DoubleUtil
								.getDoubleFromPrettyNumber(value3); // Amount
																	// readed
						if (amountReaded != null
								&& amountReaded.compareTo(amountExpected) != 0)
							jTextFieldWithPerc.setText("");
					}
				} else {
				...
				.setHorizontalGroup(jPanel21Layout
						...
						.add(jPanel21Layout
								...
								.add(jPanel21Layout
										...
										.add(jTextFieldTotalDisc,
												org.jdesktop.layout.GroupLayout.PREFERRED_SIZE,
												100,
												org.jdesktop.layout.GroupLayout.PREFERRED_SIZE))
								.add(jScrollPaneGlobDisc,
										org.jdesktop.layout.GroupLayout.PREFERRED_SIZE,
										887,
										org.jdesktop.layout.GroupLayout.PREFERRED_SIZE))
						.add(5, 5, 5)
		...
		jTabbedPaneGlobal.addTab(Constants.LANG.getString("Discounts"),
				jPanelGlbDiscounts);
		jPanelGlbReimbExp.setBackground(Constants.BKG_MAIN_COLOR);
		jScrollPaneGlobReim.getViewport().setBackground(
				Constants.BKG_MAIN_COLOR);
		...
		for (int i = 0; i < jTableGlobReim.getColumnCount(); i++) {
			jTableGlobReim.getColumnModel().getColumn(i)
					.setHeaderRenderer(new HeaderRenderer());
		}
		...
		jLabelTotalReim.setText(Constants.LANG
				.getString("TotalReimbursableExpenses")
				+ " ("
				+ Constants.LANG.getString("EuroSimbol") + ")");
		...
		jButtonReimAdd.setIcon(new ImageIcon(getClass().getResource(
				"/images/button_mini_plus.jpg")));
		...
		jButtonReimDel.setIcon(new ImageIcon(getClass().getResource(
				"/images/button_mini_minus.jpg")));
		...
		org.jdesktop.layout.GroupLayout jPanel23Layout = new org.jdesktop.layout.GroupLayout(
				jPanelGlbReimbExp);
				...
				.setHorizontalGroup(jPanel23Layout
						...
						.add(jPanel23Layout
								...
								.add(jPanel23Layout
										...
										.addPreferredGap(
												org.jdesktop.layout.LayoutStyle.RELATED)
										.add(jTextFieldTotalReim,
												org.jdesktop.layout.GroupLayout.PREFERRED_SIZE,
												100,
												org.jdesktop.layout.GroupLayout.PREFERRED_SIZE))
								.add(jScrollPaneGlobReim,
										org.jdesktop.layout.GroupLayout.PREFERRED_SIZE,
										887,
										org.jdesktop.layout.GroupLayout.PREFERRED_SIZE))
						.add(5, 5, 5)
		...
		jTabbedPaneGlobal.addTab(
				Constants.LANG.getString("ReimbursableExpenses"),
				jPanelGlbReimbExp);
		jPanelGlbCharges.setBackground(Constants.BKG_MAIN_COLOR);
		jScrollPaneGlobChar.getViewport().setBackground(
				Constants.BKG_MAIN_COLOR);
		jTableGlobChar
				.setModel(new NoEdiTableModel(
						new Object[][] {},
						new String[] {
								Constants.LANG.getString("Item"),
								Constants.LANG.getString("Rate"),
								Constants.LANG.getString("Amount")
										+ " ("
										+ Constants.LANG
												.getString("EuroSimbol") + ")" },
						new int[] {}));
		for (int i = 0; i < jTableGlobChar.getColumnCount(); i++) {
			jTableGlobChar.getColumnModel().getColumn(i)
					.setHeaderRenderer(new HeaderRenderer());
		}
		...
		jButtonCharAdd.setIcon(new ImageIcon(getClass().getResource(
				"/images/button_mini_plus.jpg")));
		...
		jButtonCharDel.setIcon(new ImageIcon(getClass().getResource(
				"/images/button_mini_minus.jpg")));
		...
		jLabelTotalChar.setText(Constants.LANG.getString("TotalCharges") + " ("
				+ Constants.LANG.getString("EuroSimbol") + ")");
				...
				.setHorizontalGroup(jPanel15Layout
						...
						.add(jPanel15Layout
								...
								.add(jPanel15Layout
										...
										.addPreferredGap(
												org.jdesktop.layout.LayoutStyle.RELATED)
										.add(jTextFieldTotalChar,
												org.jdesktop.layout.GroupLayout.PREFERRED_SIZE,
												100,
												org.jdesktop.layout.GroupLayout.PREFERRED_SIZE))
								.add(jScrollPaneGlobChar,
										org.jdesktop.layout.GroupLayout.PREFERRED_SIZE,
										887,
										org.jdesktop.layout.GroupLayout.PREFERRED_SIZE))
						.add(5, 5, 5)
		...
		jTabbedPaneGlobal.addTab(Constants.LANG.getString("Charges"),
				jPanelGlbCharges);

		jPanelGlbAttchDoc.setBackground(Constants.BKG_MAIN_COLOR);

		jScrollPaneGlobAttc.getViewport().setBackground(
				Constants.BKG_MAIN_COLOR);

		jTableGlobAttc.setModel(new NoEdiTableModel(new Object[][] {

		}, new String[] { Constants.LANG.getString("Format"),
				Constants.LANG.getString("Path"),
				Constants.LANG.getString("Description"),
				Constants.LANG.getString("IncludeInXML") }, new int[] {}));
		for (int i = 0; i < jTableGlobAttc.getColumnCount(); i++) {
			jTableGlobAttc.getColumnModel().getColumn(i)
					.setHeaderRenderer(new HeaderRenderer());
		}
		...
		jButtonAttcAdd.setIcon(new ImageIcon(getClass().getResource(
				"/images/button_mini_plus.jpg")));
		...
		jButtonAttcDel.setIcon(new ImageIcon(getClass().getResource(
				"/images/button_mini_minus.jpg")));
		...
		jTextAreaXMLExtAddData.setBorder(new LineBorder(
				Constants.SELECTION_COLOR, 1, true));
		jTextAreaXMLExtAddData.setEditable(true);
		jScrollPaneXMLAddData.getViewport().setBackground(
				Constants.BKG_MAIN_COLOR);
		jScrollPaneXMLAddData.setBorder(new LineBorder(
				Constants.SELECTION_COLOR, 1, true));
		jScrollPaneXMLAddData.setViewportView(jTextAreaXMLExtAddData);

		jButtonXMLAddData.setIcon(new ImageIcon(getClass().getResource(
				"/images/button_mini_accept.jpg")));
		...
		jButtonXMLAddData
				.addActionListener(new java.awt.event.ActionListener() {
					...
					private void verifyXML(ActionEvent evt) {
						if (jTextAreaXMLExtAddData.getText() == null
								|| "".equals(jTextAreaXMLExtAddData.getText()
										.trim())) {
							error = "";
							jButtonXMLAddDataZoom.setIcon(new ImageIcon(
							...
							try {
								...
								error = ve.getMessage();
								((GenerateInvoice321Window) jButtonXMLAddData
										.getTopLevelAncestor()).showMessage(
										Constants.LANG
												.getString("NOOKMessageExtensionAdded"),
										Constants.ERROR_MSG_COLOR);
							...
							if (f != null) {
								try {
									ValidatorUtil vu = ValidatorUtil
											.getInstance();
									...
									((GenerateInvoice321Window) jButtonXMLAddData
											.getTopLevelAncestor()).showMessage(
											Constants.LANG
													.getString("OKMessageExtensionAdded"),
											Constants.OK_MSG_COLOR);
								} catch (ValidationException ve) {
									jButtonXMLAddDataZoom
											.setIcon(new ImageIcon(
													getClass()
		...
		jButtonXMLAddDataZoom
				.addActionListener(new java.awt.event.ActionListener() {
					public void actionPerformed(java.awt.event.ActionEvent evt) {
						InputExtensionDialog ied = new InputExtensionDialog(
								(JFrame) jTextAreaXMLExtAddData
										.getTopLevelAncestor(), true,
								jTextAreaXMLExtAddData.getText(), error);
						ied.setVisible(true);
		...
		jLabelXMLAddData = new JLabel();
		jLabelXMLAddData.setText(Constants.LANG
		...
		org.jdesktop.layout.GroupLayout jPanel24Layout = new org.jdesktop.layout.GroupLayout(
				jPanelGlbAttchDoc);
		...
		jPanel24Layout
				.setHorizontalGroup(jPanel24Layout
						...
						.add(jPanel24Layout
								.createParallelGroup(
										org.jdesktop.layout.GroupLayout.TRAILING)
								.add(jButtonXMLAddData,
										org.jdesktop.layout.GroupLayout.PREFERRED_SIZE,
										org.jdesktop.layout.GroupLayout.PREFERRED_SIZE,
										Short.MAX_VALUE)
								.add(jButtonXMLAddDataZoom,
										org.jdesktop.layout.GroupLayout.PREFERRED_SIZE,
										org.jdesktop.layout.GroupLayout.PREFERRED_SIZE,
		...
		jTabbedPaneGlobal.addTab(Constants.LANG.getString("Attachments"),
				jPanelGlbAttchDoc);
		...
		jLabelTotalPayAmou.setText(Constants.LANG.getString("Amount") + " ("
				+ Constants.LANG.getString("EuroSimbol") + ")");
					...
					try {
						...
						((JTextField) evt.getSource()).setText(DoubleUtil
								.roundWithPrecisionDoubleFormatted(
										doubleObj,
										Integer.parseInt(decimalProps
												.getProperty("item_payment_amount"))));
					} catch (NumberFormatException nfe) {
						if (((JTextField) evt.getSource()).getText() != null
								&& !"".equals(((JTextField) evt.getSource())
										.getText().trim())) {
							((JTextField) evt.getSource()).setText("");
							((GenerateInvoice321Window) jTextFieldPaymentAmo
									.getTopLevelAncestor()).showMessage(
									Constants.LANG
											.getString("NOOKParamNumericFormat"),
									Constants.ERROR_MSG_COLOR);
		...
		String[] arrayPaymentMeansModel = Constants.APP_PROP.getProperty(
				"paymentMeansCombo" + Constants.FACTURAE31).split(";");
		for (int i = 0; i < arrayPaymentMeansModel.length; i++)
			arrayPaymentMeansModel[i] = Constants.LANG
					.getString(arrayPaymentMeansModel[i]);
		DefaultComboBoxModel PaymentMeansModel = new DefaultComboBoxModel(
				arrayPaymentMeansModel);
		...
		jLabelTotalPayRefIssuer.setText(Constants.LANG
				.getString("PaymentReferenceIssuer"));

		jLabelTotalPayRefReceiver.setFont(Constants.FONT_PLAIN);
		jLabelTotalPayRefReceiver.setForeground(Constants.FONT_COLOR);
		jLabelTotalPayRefReceiver.setText(Constants.LANG
				.getString("PaymentReferenceReceiver"));
		...
		jTextFieldPaymentRefIssuer.addFocusListener(new FocusAdapter() {
			public void focusLost(FocusEvent evt) {
				...
				if (value.length() > 60) {
					((JTextField) evt.getSource()).setText("");
					((GenerateInvoice321Window) jTextFieldPaymentRefIssuer
							.getTopLevelAncestor()).showMessage(
							Constants.LANG.getString("NOOKParamTooHigh"),
							Constants.ERROR_MSG_COLOR);
				}
			}
		});
		...
		jTextFieldPaymentRefReceiver.addFocusListener(new FocusAdapter() {
			public void focusLost(FocusEvent evt) {
				...
				if (value.length() > 60) {
					((JTextField) evt.getSource()).setText("");
					((GenerateInvoice321Window) jTextFieldPaymentRefReceiver
							.getTopLevelAncestor()).showMessage(
							Constants.LANG.getString("NOOKParamTooHigh"),
							Constants.ERROR_MSG_COLOR);
				}
		...
		jComboBoxPayment.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent evt) {
				...
				if (((JComboBox) evt.getSource()) != null
						&& ((JComboBox) evt.getSource()).getSelectedItem() != null
						&& !"".equals(((JComboBox) evt.getSource())
								.getSelectedItem().toString())) {
					String valueId = ((JComboBox) evt.getSource())
							.getSelectedItem().toString().split(":")[1].trim();
					...
					if (ls != null && ls.size() > 0 && ls.get(0) != null) {
						...
						if (values[0] != null && values[0] instanceof Date)
							jCalendarComboBoxPaymDate.getModel().setValue(
									(Date) (values[0]));
						if (values[1] != null)
							jTextFieldPaymentAmo.setText(String
									.valueOf(values[1]));
						if (values[2] != null
								&& !"0".equals(String.valueOf(values[2])))
							jComboBoxPayMean.setSelectedIndex(Integer
									.parseInt(String.valueOf(values[2])) - 1);
						if (values[3] != null)
							jTextFieldPaymentRefIssuer.setText(String
									.valueOf(values[3]));
						if (values[5] != null) {
							String query2 = "SELECT ACCOUNT_ID,IBAN FROM ACCOUNT WHERE ACCOUNT_ID='"
									+ values[5] + "'";
							SQLQuery s2 = FacturaeManager.getInstance()
									.executeQuery(query2);
							List<?> ls2 = s2.list();
							if (ls2 != null && ls2.size() > 0
								...
								if (values2[0] != null && values2[1] != null)
									jComboBoxPayAccountD
											.setSelectedItem(values2[0] + ": "
													+ values2[1]);
							}
						}
						if (values[6] != null) {
							String query3 = "SELECT ACCOUNT_ID,IBAN FROM ACCOUNT WHERE ACCOUNT_ID='"
									+ values[6] + "'";
							SQLQuery s3 = FacturaeManager.getInstance()
									.executeQuery(query3);
							List<?> ls3 = s3.list();
							if (ls3 != null && ls3.size() > 0
								...
								if (values2[0] != null && values2[1] != null)
									jComboBoxPayAccountC
											.setSelectedItem(values2[0] + ": "
													+ values2[1]);
							}
						}
						if (values[7] != null)
							jTextFieldPaymentRefReceiver.setText(String
									.valueOf(values[3]));
					}
				}
			}
		...
		((JFormattedTextField) ((JSpinner) jCalendarComboBoxPaymDate
				.getComponent(0)).getEditor().getComponent(0))
				.addFocusListener(new FocusAdapter() {
					public void focusLost(FocusEvent evt) {
						if (!((JFormattedTextField) evt.getSource())
								.isEditValid())
							((GenerateInvoice321Window) jCalendarComboBoxPaymDate
									.getTopLevelAncestor()).showMessage(
									Constants.LANG
											.getString("NOOKMessageDateNotValid"),
									Constants.ERROR_MSG_COLOR);
					}
				});
		...
		jTextAreaPayExist.setText(Constants.LANG
				.getString("MessageExistingPayment"));
		...
		jPanelGlbPaymentGen2Layout
				.setHorizontalGroup(jPanelGlbPaymentGen2Layout
						.createParallelGroup(
								org.jdesktop.layout.GroupLayout.LEADING)
						.add(jPanelGlbPaymentGen2Layout
								.createSequentialGroup()
								.add(jPanelGlbPaymentGen2Layout
										.createParallelGroup(
												org.jdesktop.layout.GroupLayout.TRAILING)
								...
								.add(jPanelGlbPaymentGen2Layout
										.createParallelGroup(
												org.jdesktop.layout.GroupLayout.TRAILING)
										.add(jCalendarComboBoxPaymDate,
												org.jdesktop.layout.GroupLayout.DEFAULT_SIZE,
												160, Short.MAX_VALUE)
										.add(jComboBoxPayMean,
												org.jdesktop.layout.GroupLayout.DEFAULT_SIZE,
												160, Short.MAX_VALUE))
								.add(20, 20, 20)
								.add(jPanelGlbPaymentGen2Layout
										...
										.add(org.jdesktop.layout.GroupLayout.CENTER,
												jPanelGlbPaymentGen2Layout
														...
														.add(jTextFieldPaymentRefReceiver,
																...
																60,
																org.jdesktop.layout.GroupLayout.PREFERRED_SIZE))
										.add(jTextFieldPaymentAmo,
												org.jdesktop.layout.GroupLayout.DEFAULT_SIZE,
												100, Short.MAX_VALUE))
								.add(20, 20, 20)));

		jPanelGlbPaymentGen2Layout
				.setVerticalGroup(jPanelGlbPaymentGen2Layout
						.createParallelGroup(
								org.jdesktop.layout.GroupLayout.LEADING)
						.add(jPanelGlbPaymentGen2Layout
								.createSequentialGroup()
		...
		jTabbedPaneGlbPayment.addTab(Constants.LANG.getString("General"),
				jPanelGlbPaymentGen2);
		...
		jTextFieldPayDIBAN.addFocusListener(new FocusAdapter() {
			public void focusLost(FocusEvent evt) {
				...
				if (value.length() < 5 && value.length() > 0) {
					((JTextField) evt.getSource()).setText("");
					((GenerateInvoice321Window) jTextFieldPayDIBAN
							.getTopLevelAncestor()).showMessage(
							Constants.LANG.getString("NOOKParamTooLow"),
							Constants.ERROR_MSG_COLOR);
				}
				if (value.length() > 34) {
					((JTextField) evt.getSource()).setText("");
		...
		jTextFieldPayDBank.addFocusListener(new FocusAdapter() {
			public void focusLost(FocusEvent evt) {
				...
				if (value.length() > 60) {
					((JTextField) evt.getSource()).setText("");
					((GenerateInvoice321Window) jTextFieldPayDBank
							.getTopLevelAncestor()).showMessage(
							Constants.LANG.getString("NOOKParamTooHigh"),
							Constants.ERROR_MSG_COLOR);
				}
		...
		jTextFieldPayDBran.addFocusListener(new FocusAdapter() {
			public void focusLost(FocusEvent evt) {
				...
				if (value.length() > 60) {
					((JTextField) evt.getSource()).setText("");
					((GenerateInvoice321Window) jTextFieldPayDBran
							.getTopLevelAncestor()).showMessage(
							Constants.LANG.getString("NOOKParamTooHigh"),
							Constants.ERROR_MSG_COLOR);
				}
		...
		jTextFieldPayDAddr.addFocusListener(new FocusAdapter() {
			public void focusLost(FocusEvent evt) {
				...
				if (value.length() > 80) {
					((JTextField) evt.getSource()).setText("");
					((GenerateInvoice321Window) jTextFieldPayDAddr
							.getTopLevelAncestor()).showMessage(
							Constants.LANG.getString("NOOKParamTooHigh"),
							Constants.ERROR_MSG_COLOR);
				}
		...
		jTextFieldPayDPCod.addFocusListener(new FocusAdapter() {
			public void focusLost(FocusEvent evt) {
				...
				if (value != null)
					value = value.trim();
				// Postal code may be a 5 length string (But numeric format
				// [0-9]*)
				if (value.length() != 5 && value.length() != 0) {
					((JTextField) evt.getSource()).setText("");
					((GenerateInvoice321Window) jTextFieldPayDPCod
							.getTopLevelAncestor()).showMessage(
							Constants.LANG.getString("NOOKParamPostCodeLength"),
							Constants.ERROR_MSG_COLOR);
					return;
				}
				try {
					...
					((GenerateInvoice321Window) jTextFieldPayDPCod
							.getTopLevelAncestor()).showMessage(
							Constants.LANG.getString("NOOKParamNumericFormat"),
							Constants.ERROR_MSG_COLOR);
					return;
				}
			}
		...
		jTextFieldPayDCity.addFocusListener(new FocusAdapter() {
			public void focusLost(FocusEvent evt) {
				...
				if (value.length() > 50) {
					((JTextField) evt.getSource()).setText("");
					((GenerateInvoice321Window) jTextFieldPayDCity
							.getTopLevelAncestor()).showMessage(
							Constants.LANG.getString("NOOKParamTooHigh"),
							Constants.ERROR_MSG_COLOR);
				}
		...
		jTextFieldPayDProv.addFocusListener(new FocusAdapter() {
			public void focusLost(FocusEvent evt) {
				...
				if (value.length() > 20) {
					((JTextField) evt.getSource()).setText("");
					((GenerateInvoice321Window) jTextFieldPayDProv
							.getTopLevelAncestor()).showMessage(
							Constants.LANG.getString("NOOKParamTooHigh"),
							Constants.ERROR_MSG_COLOR);
				}
			...
			public void actionPerformed(ActionEvent evt) {
				...
				if (((JComboBox) evt.getSource()) != null
					...
					if (ls != null && ls.size() > 0 && ls.get(0) != null) {
						Object[] values = (Object[]) ls.get(0);
						if (values[0] != null)
							jTextFieldPayDIBAN.setText(String
									.valueOf(values[0]));
						if (values[1] != null)
							jTextFieldPayDBank.setText(String
									.valueOf(values[1]));
						if (values[2] != null)
							jTextFieldPayDBran.setText(String
									.valueOf(values[2]));
						if (values[3] != null) {
							...
							if (ls2 != null && ls2.size() > 0
								...
								Object[] values2 = (Object[]) ls2.get(0);
								if (values2[0] != null)
									jTextFieldPayDAddr.setText(String
											.valueOf(values2[0]));
								if (values2[1] != null)
									jTextFieldPayDCity.setText(String
											.valueOf(values2[1]));
								if (values2[2] != null)
									jTextFieldPayDPCod.setText(String
											.valueOf(values2[2]));
								if (values2[3] != null)
									jTextFieldPayDProv.setText(String
											.valueOf(values2[3]));
								if (values2[4] != null)
									jComboBoxPayDCoun.setSelectedIndex(ComboUtil
											.calculateComboIndex(
													Constants.COUNTRIES31,
													String.valueOf(values2[4])));
							}
						}
		...
		jTextAreaAccExist.setText(Constants.LANG
				.getString("MessageExistingAccount"));
		...
		org.jdesktop.layout.GroupLayout jPanel26Layout = new org.jdesktop.layout.GroupLayout(
				jPanelGlbPaymentAccDebited);
		...
		jTabbedPaneGlbPayment.addTab(
				Constants.LANG.getString("AccountToBeDebited"),
				jPanelGlbPaymentAccDebited);
		...
		jTextFieldPayCIBAN.addFocusListener(new FocusAdapter() {
			public void focusLost(FocusEvent evt) {
				...
				if (value.length() < 5 && value.length() > 0) {
					((JTextField) evt.getSource()).setText("");
					((GenerateInvoice321Window) jTextFieldPayCIBAN
							.getTopLevelAncestor()).showMessage(
							Constants.LANG.getString("NOOKParamTooLow"),
							Constants.ERROR_MSG_COLOR);
				}
		...
		jTextFieldPayCBank.addFocusListener(new FocusAdapter() {
			public void focusLost(FocusEvent evt) {
				...
				if (value.length() > 60) {
					((JTextField) evt.getSource()).setText("");
					((GenerateInvoice321Window) jTextFieldPayCBank
							.getTopLevelAncestor()).showMessage(
							Constants.LANG.getString("NOOKParamTooHigh"),
							Constants.ERROR_MSG_COLOR);
				}
		...
		jTextFieldPayCBran.addFocusListener(new FocusAdapter() {
			public void focusLost(FocusEvent evt) {
				...
				if (value.length() > 60) {
					((JTextField) evt.getSource()).setText("");
					((GenerateInvoice321Window) jTextFieldPayCBran
							.getTopLevelAncestor()).showMessage(
							Constants.LANG.getString("NOOKParamTooHigh"),
							Constants.ERROR_MSG_COLOR);
				}
		...
		jTextFieldPayCAddr.addFocusListener(new FocusAdapter() {
			public void focusLost(FocusEvent evt) {
				...
				if (value.length() > 80) {
					((JTextField) evt.getSource()).setText("");
					((GenerateInvoice321Window) jTextFieldPayCAddr
							.getTopLevelAncestor()).showMessage(
							Constants.LANG.getString("NOOKParamTooHigh"),
							Constants.ERROR_MSG_COLOR);
				}
		...
		jTextFieldPayCCity.addFocusListener(new FocusAdapter() {
			public void focusLost(FocusEvent evt) {
				...
				if (value.length() > 50) {
					((JTextField) evt.getSource()).setText("");
					((GenerateInvoice321Window) jTextFieldPayCCity
							.getTopLevelAncestor()).showMessage(
							Constants.LANG.getString("NOOKParamTooHigh"),
							Constants.ERROR_MSG_COLOR);
				}
		...
		jTextFieldPayCPCod.addFocusListener(new FocusAdapter() {
			public void focusLost(FocusEvent evt) {
				String value = ((JTextField) evt.getSource()).getText();
				// Post code may be a 5 length string (But numeric format
				// [0-9]*)
				if (value != null)
					value = value.trim();
				if (value.length() != 5 && value.length() != 0) {
					((JTextField) evt.getSource()).setText("");
					((GenerateInvoice321Window) jTextFieldPayCPCod
							.getTopLevelAncestor()).showMessage(
							Constants.LANG.getString("NOOKParamPostCodeLength"),
							Constants.ERROR_MSG_COLOR);
					return;
		...
		jTextFieldPayCProv.addFocusListener(new FocusAdapter() {
			public void focusLost(FocusEvent evt) {
				...
				if (value.length() > 20) {
					((JTextField) evt.getSource()).setText("");
					((GenerateInvoice321Window) jTextFieldPayCProv
							.getTopLevelAncestor()).showMessage(
							Constants.LANG.getString("NOOKParamTooHigh"),
							Constants.ERROR_MSG_COLOR);
				}
		...
		DefaultComboBoxModel CountryModel2 = new DefaultComboBoxModel(
				Constants.COUNTRIES31.toArray());
			...
			public void actionPerformed(ActionEvent evt) {
				...
				if (((JComboBox) evt.getSource()) != null
					...
					if (ls != null && ls.size() > 0 && ls.get(0) != null) {
						Object[] values = (Object[]) ls.get(0);
						if (values[0] != null)
							jTextFieldPayCIBAN.setText(String
									.valueOf(values[0]));
						if (values[1] != null)
							jTextFieldPayCBank.setText(String
									.valueOf(values[1]));
						if (values[2] != null)
							jTextFieldPayCBran.setText(String
									.valueOf(values[2]));
						if (values[3] != null) {
							String query2 = "SELECT ADDRESS,TOWN,POST_CODE,PROVINCE,COUNTRY FROM ADDRESS WHERE ADDRESS_ID='"
									+ String.valueOf(values[3]) + "'";
							...
							if (ls2 != null && ls2.size() > 0
								...
								Object[] values2 = (Object[]) ls2.get(0);
								if (values2[0] != null)
									jTextFieldPayCAddr.setText(String
											.valueOf(values2[0]));
								if (values2[1] != null)
									jTextFieldPayCCity.setText(String
											.valueOf(values2[1]));
								if (values2[2] != null)
									jTextFieldPayCPCod.setText(String
											.valueOf(values2[2]));
								if (values2[3] != null)
									jTextFieldPayCProv.setText(String
											.valueOf(values2[3]));
								if (values2[4] != null)
									jComboBoxPayCCoun.setSelectedIndex(ComboUtil
											.calculateComboIndex(
													Constants.COUNTRIES31,
													String.valueOf(values2[4])));
							}
		...
		jTextAreaAccExist2.setText(Constants.LANG
				.getString("MessageExistingAccount"));
		...
		org.jdesktop.layout.GroupLayout jPanel27Layout = new org.jdesktop.layout.GroupLayout(
				jPanelGlbPaymentAccCharged);
		...
		jPanel27Layout
				.setVerticalGroup(jPanel27Layout
						.createParallelGroup(
								org.jdesktop.layout.GroupLayout.LEADING)
						.add(jPanel27Layout
								.createSequentialGroup()
		...
		jTabbedPaneGlbPayment.addTab(
				Constants.LANG.getString("AccountToBeCredited"),
				jPanelGlbPaymentAccCharged);

		org.jdesktop.layout.GroupLayout jPanel28Layout = new org.jdesktop.layout.GroupLayout(
				jPanelGlbPayment);
		...
		jPanel28Layout.setHorizontalGroup(jPanel28Layout.createParallelGroup(
				...
				jPanel28Layout
						...
						.addPreferredGap(
								org.jdesktop.layout.LayoutStyle.RELATED)
						.add(jTabbedPaneGlbPayment,
								org.jdesktop.layout.GroupLayout.PREFERRED_SIZE,
								931,
								org.jdesktop.layout.GroupLayout.PREFERRED_SIZE)
						.add(13, 13, 13)));
		jPanel28Layout
				.setVerticalGroup(jPanel28Layout
						.createParallelGroup(
								org.jdesktop.layout.GroupLayout.LEADING)
						.add(jPanel28Layout
								.createSequentialGroup()
		...
		jTabbedPaneGlobal.addTab(Constants.LANG.getString("PaymentDetails"),
				jPanelGlbPayment);
		...
		jLabelTotalWithAmou.setText(Constants.LANG.getString("Amount") + " ("
				+ Constants.LANG.getString("EuroSimbol") + ")");
		...
		org.jdesktop.layout.GroupLayout jPanel29Layout = new org.jdesktop.layout.GroupLayout(
				jPanelGlbWhithhelds);
		...
		jPanel29Layout.setHorizontalGroup(jPanel29Layout.createParallelGroup(
				...
				jPanel29Layout
						...
						.addPreferredGap(
								org.jdesktop.layout.LayoutStyle.RELATED)
						.add(jTextFieldWithAmou,
								org.jdesktop.layout.GroupLayout.PREFERRED_SIZE,
								118,
								org.jdesktop.layout.GroupLayout.PREFERRED_SIZE)
						.addContainerGap(124, Short.MAX_VALUE)));
		jPanel29Layout
				.setVerticalGroup(jPanel29Layout
						.createParallelGroup(
								org.jdesktop.layout.GroupLayout.LEADING)
						.add(jPanel29Layout
								.createSequentialGroup()
		...
		jTabbedPaneGlobal.addTab(Constants.LANG.getString("AmountsWithheld"),
				jPanelGlbWhithhelds);

		jPanelGlbLiterals.setBackground(Constants.BKG_MAIN_COLOR);

		if (Constants.LEGAL_LITERALS != null) {
			DefaultComboBoxModel LegalLiteralModel = new DefaultComboBoxModel(
					Constants.LEGAL_LITERALS.toArray());
		...
		ListSelectionListener listSelectionListener = new ListSelectionListener() {
			public void valueChanged(ListSelectionEvent listSelectionEvent) {
				...
				ComboOption legalLiteral = (ComboOption) jComboBoxLegalLiterals
						.getItemAt(selection);
		...
		jButtonLitItemAdd.setIcon(new ImageIcon(getClass().getResource(
				"/images/button_list_add.jpg")));
		...
		jButtonLitItemAdd
				.addActionListener(new java.awt.event.ActionListener() {
					public void actionPerformed(java.awt.event.ActionEvent evt) {
						if (!literalsListDestModel.contains(jTextAreaLegal
								.getText()))
							literalsListDestModel.addElement(jTextAreaLegal
									.getText());
					}
				});
		jButtonLitItemRemove.setIcon(new ImageIcon(getClass().getResource(
				"/images/button_list_remove.jpg")));
		...
		jButtonLitItemRemove
				.addActionListener(new java.awt.event.ActionListener() {
					public void actionPerformed(java.awt.event.ActionEvent evt) {
						if (jListLiteralsDest.getSelectedIndices().length > 0) {
							int[] currentIndexes = jListLiteralsDest
									.getSelectedIndices();
							int[] selectedIndices = jListLiteralsDest
									.getSelectedIndices();
							for (int i = selectedIndices.length - 1; i >= 0; i--) {
								currentIndexes = jListLiteralsDest
										.getSelectedIndices();
								literalsListDestModel
										.removeElementAt(currentIndexes[i]);
							}
						}
					}
				});
		org.jdesktop.layout.GroupLayout jPanel30Layout = new org.jdesktop.layout.GroupLayout(
				jPanelGlbLiterals);
		jPanelGlbLiterals.setLayout(jPanel30Layout);
		jPanel30Layout
				.setHorizontalGroup(jPanel30Layout
										...
										.add(jPanel30Layout
												...
												.add(jPanel30Layout
														...
														.add(jButtonLitItemRemove,
																...
																0,
																Short.MAX_VALUE)))
										.add(jScrollPaneLitEdit,
												GroupLayout.PREFERRED_SIZE,
												420, GroupLayout.PREFERRED_SIZE))
								.add(jScrollPaneLitDest, 0, 380,
										Short.MAX_VALUE)));
		jPanel30Layout
				.setVerticalGroup(jPanel30Layout
		...
		jTabbedPaneGlobal.addTab(Constants.LANG.getString("LegalLiterals"),
				jPanelGlbLiterals);
		jScrollPaneItems.getViewport().setBackground(Constants.BKG_MAIN_COLOR);
		jScrollPaneItems.setBorder(new LineBorder(Constants.SECOND_COLOR, 2,
				false));
		jTableItems
				.setModel(new NoEdiTableModel(
						new Object[][] {},
						new String[] {
								Constants.LANG.getString("Id"),
								Constants.LANG.getString("Description"),
								...
								Constants.LANG.getString("Amount")
										...
										+ Constants.LANG
												.getString("EuroSimbol") + ")",
								Constants.LANG.getString("TaxesOutputs")
										+ " ("
										+ Constants.LANG
												.getString("EuroSimbol") + ")",
								Constants.LANG.getString("TaxesWithheld")
										+ " ("
										+ Constants.LANG
												.getString("EuroSimbol") + ")",
								Constants.LANG.getString("TotalAmount")
										+ " ("
										+ Constants.LANG
												.getString("EuroSimbol") + ")" },
						new int[] {}));
		...
		for (int i = 0; i < jTableItems.getColumnCount(); i++) {
			jTableItems.getColumnModel().getColumn(i)
					.setHeaderRenderer(new HeaderRenderer());
		}
		...
		jLabelTOTTaxBase2
				.setText(Constants.LANG.getString("AmountBeforeTaxes"));
		...
		jLabelTOTReimb
				.setText(Constants.LANG.getString("ReimbursableExpenses"));
		...
		jTextFieldResults[9].addFocusListener(new FocusAdapter() {
			public void focusLost(FocusEvent evt) {
				if (((JTextField) evt.getSource()) != null
						&& ((JTextField) evt.getSource()).getText() != null) {
					if (!((JTextField) evt.getSource()).getText().trim()
							.equals("")) {
						try {
							Double d = Double.valueOf(((JTextField) evt
									.getSource()).getText().replace(",","."));
							if (d.doubleValue() < 0) {
								d = 0.0;
								((JTextField) evt.getSource())
										.setForeground(Constants.BKG_ERROR_COLOR);
								((GenerateInvoice321Window) jTextFieldWithItem
										.getTopLevelAncestor()).showMessage(
										Constants.LANG
												.getString("NOOKParamNegative"),
							...
							((JTextField) evt.getSource()).setText(DoubleUtil
									.roundWithPrecisionDoubleFormatted(
											d,
											Integer.parseInt(decimalProps
													.getProperty("item_financial_expenses"))));
						} catch (NumberFormatException nfe) {
							((JTextField) evt.getSource()).setText("");
							((JTextField) evt.getSource())
									.setForeground(Constants.BKG_ERROR_COLOR);
		...
		jLabelTOTFinancial.setText(Constants.LANG
				.getString("FinancialExpenses"));
		...
		jButtonItemAdd.setIcon(new ImageIcon(getClass().getResource(
				"/images/button_mini_plus.jpg")));
		...
		jButtonItemDel.setIcon(new ImageIcon(getClass().getResource(
				"/images/button_mini_minus.jpg")));
		...
		jTextAreaTOTXMLExtAddData.setBorder(new LineBorder(
				Constants.SELECTION_COLOR, 1, true));
		jTextAreaTOTXMLExtAddData.setEditable(true);
		jScrollPaneTOTXMLAddData.getViewport().setBackground(
				Constants.BKG_MAIN_COLOR);
		jScrollPaneTOTXMLAddData.setBorder(new LineBorder(
				Constants.SELECTION_COLOR, 1, true));
		jScrollPaneTOTXMLAddData.setViewportView(jTextAreaTOTXMLExtAddData);

		jButtonTOTXMLAddData.setIcon(new ImageIcon(getClass().getResource(
				"/images/button_mini_accept.jpg")));
				...
				.addActionListener(new java.awt.event.ActionListener() {
					...
					private void verifyXML(ActionEvent evt) {
						if (jTextAreaTOTXMLExtAddData.getText() == null
								|| "".equals(jTextAreaTOTXMLExtAddData
										.getText().trim())) {
							error2 = "";
							jButtonTOTXMLAddDataZoom.setIcon(new ImageIcon(
									getClass().getResource(
											"/images/button_mini_zoom.jpg")));
						} else {
							// Valid Invoice 3.1
							...
							try {
								isExt = new ByteArrayInputStream(
										jTextAreaTOTXMLExtAddData.getText()
												.getBytes("UTF-8"));
							} catch (UnsupportedEncodingException e) {
								logger.error("Error creating a bytes array with extension data. The encoding is unsupported: "
										+ e.getMessage());
							...
							try {
								...
								jButtonTOTXMLAddDataZoom
										.setIcon(new ImageIcon(
												getClass()
														.getResource(
																"/images/button_mini_zoom_error.jpg")));
								error2 = ve.getMessage();
								((GenerateInvoice321Window) jButtonTOTXMLAddData
										.getTopLevelAncestor()).showMessage(
										Constants.LANG
												.getString("NOOKMessageExtensionAdded"),
										Constants.ERROR_MSG_COLOR);
		...
		jButtonTOTXMLAddDataZoom.setIcon(new ImageIcon(getClass().getResource(
				"/images/button_mini_zoom.jpg")));
		jButtonTOTXMLAddDataZoom.setToolTipText(Constants.LANG
				.getString("Zoom"));
		...
		jButtonTOTXMLAddDataZoom
				.addActionListener(new java.awt.event.ActionListener() {
					public void actionPerformed(java.awt.event.ActionEvent evt) {
						InputExtensionDialog ied = new InputExtensionDialog(
								(JFrame) jTextAreaTOTXMLExtAddData
										.getTopLevelAncestor(), true,
								jTextAreaTOTXMLExtAddData.getText(), error2);
						ied.setVisible(true);
		...
		jLabelTOTXMLAddData = new JLabel();
		jLabelTOTXMLAddData.setText(Constants.LANG.getString("Extension")
				+ " (XML)");
		...
		this.setBorder(BorderFactory.createTitledBorder(new LineBorder(
				Constants.SELECTION_COLOR, 1, true), Constants.LANG
				.getString("InvoiceDetailData"),
				TitledBorder.DEFAULT_JUSTIFICATION,
				TitledBorder.DEFAULT_POSITION, Constants.FONT_BOLD,
	...
	private void loadDecimalConfiguration() {
		try {
			decimalProps.load(this.getClass().getResourceAsStream(
					Constants.APP_PROP.getProperty("fact" + codeVersion
							+ "d_resource")));
		} catch (IOException ioe) {
			logger.error("It is not possible to obtain the decimals 3.2.1 properties file: "
					+ ioe.getMessage());
		}
	}
	...
	private void loadDBData() {
		...
		try {
			s = FacturaeManager.getInstance().executeQuery(
					"SELECT PAYMENT_ID FROM PAYMENT_DETAIL");
			List<?> ls = s.list();
			payments = new DefaultComboBoxModel();
			...
			if (ls != null && ls.size() > 0 && ls.get(0) != null) {
				...
				for (int i = 0; i < count; i++)
					payments.addElement(new String(Constants.LANG
							.getString("PaymentDetails")
							+ ": "
							+ ((Integer) ls.get(i)).toString()));
				payments.addElement("");
			}
			s = FacturaeManager.getInstance().executeQuery(
					"SELECT ACCOUNT_ID,IBAN FROM ACCOUNT");
			ls = s.list();
			accounts = new DefaultComboBoxModel();
			accounts2 = new DefaultComboBoxModel();
			if (ls != null && ls.size() > 0 && ls.get(0) != null) {
			...
			logger.error("An error occurred when loading the database information: "
					+ e.getMessage());
		}
	}
	...
	private void addDiscount(java.awt.event.ActionEvent evt) {
		...
		if (tibString != null && !"".equals(tibString)) {
			try {
				...
				logger.error("An number format exception occurred while a discount was being added: "
						+ e.getMessage());
				((GenerateInvoice321Window) this.getTopLevelAncestor())
						.showMessage(Constants.LANG
								.getString("NOOKParamNumericFormat"),
								Constants.ERROR_MSG_COLOR);
				return;
			}
		}
		if ((tib == null || "".equals(tib)) || (tot == null || "".equals(tot))) { // If
																					// gross
																					// amount
																					// or
																					// total
			...
			((GenerateInvoice321Window) this.getTopLevelAncestor())
					.showMessage(
							Constants.LANG.getString("NOOKParamItemRequired"),
							Constants.ERROR_MSG_COLOR);
			return;
		}
		InputDiscountDialog igd = new InputDiscountDialog(
				(JFrame) jPanelGlbDiscounts.getTopLevelAncestor(), true, tot,
				tib, Constants.VERSION321, true); // Tot is the source, tib is
													// the value that may not
													// exceed
		igd.setVisible(true);
		...
		if (result == null || result.length != 3) {
			((GenerateInvoice321Window) this.getTopLevelAncestor())
					.showMessage(
							Constants.LANG.getString("OKMessageCanceledByUser"),
							Constants.OK_MSG_COLOR);
			return;
		} else
			((GenerateInvoice321Window) this.getTopLevelAncestor())
					.showMessage(
							Constants.LANG.getString("OKMessageDiscountAdded"),
							Constants.OK_MSG_COLOR);
		String item = result[0];
		String rate = result[1];
		String amount = result[2];
		if (item != null && !"".equals(item)) {
			((NoEdiTableModel) jTableGlobDisc.getModel()).addRow(new Object[] {
					item, rate, amount });
			Double totDiscount = 0.00;
			if (jTextFieldTotalDisc.getText() != null
					&& !jTextFieldTotalDisc.getText().trim().equals(""))
				totDiscount = Double.valueOf(jTextFieldTotalDisc.getText().replace(",","."));
			jTextFieldTotalDisc.setText(DoubleUtil
					.roundWithPrecisionDoubleFormatted((totDiscount + Double
							.valueOf(amount)), Integer.parseInt(decimalProps
							.getProperty("item_discount"))));
	...
	private void delDiscount(java.awt.event.ActionEvent evt) {
		...
		if (row >= 0) {
			...
			try {
				if (jTextFieldTotalDisc != null
						&& jTextFieldTotalDisc.getText() != null
						&& !jTextFieldTotalDisc.getText().trim().equals(""))
					totalDiscount = DoubleUtil
							.getDoubleFromPrettyNumber(jTextFieldTotalDisc
									.getText());
				jTextFieldTotalDisc
						.setText(DoubleUtil.roundWithPrecisionDoubleFormatted(
								(totalDiscount - DoubleUtil
										.getDoubleFromPrettyNumber(((NoEdiTableModel) jTableGlobDisc
												.getModel()).getValueAt(row, 2)
												.toString())), Integer
										.parseInt(decimalProps
												.getProperty("item_discount"))));
			} catch (NumberFormatException e) {
			...
			((GenerateInvoice321Window) this.getTopLevelAncestor())
					.showMessage(Constants.LANG
							.getString("OKMessageDiscountDeleted"),
							Constants.OK_MSG_COLOR);
		} else
			((GenerateInvoice321Window) this.getTopLevelAncestor())
					.showMessage(
							Constants.LANG.getString("NOOKForDeleteMessage"),
							Constants.ERROR_MSG_COLOR);
	}

	private void addReim(java.awt.event.ActionEvent evt) {
		// Total gross amount is actualized. If gross amount does not have a
		// value then return
		String tibString = jTextFieldResults[0].getText();
		if (tibString == null || "".equals(tibString.trim())) {
			((GenerateInvoice321Window) this.getTopLevelAncestor())
					.showMessage(
							"Debe facturar algun concepto antes de aplicar suplidos",
							Constants.ERROR_MSG_COLOR);
			return;
		}
		InputReimDialog igd = new InputReimDialog(
				(JFrame) jPanelGlbDiscounts.getTopLevelAncestor(), true,
				codeVersion);
		igd.setVisible(true);
		String[] result = igd.getValues();
		if (result == null || result.length != 7) {
			((GenerateInvoice321Window) this.getTopLevelAncestor())
					.showMessage(
							Constants.LANG.getString("NOOKIncompleteForm"),
							Constants.ERROR_MSG_COLOR);
			return;
		}
		String amount = result[6];
		if (amount != null && !"".equals(amount)) {// If not, it was canceled by
													// user
			...
			jTextFieldTotalReim.setText(DoubleUtil
					.roundWithPrecisionDoubleFormatted(
							(totValue + doubleAmount),
							Integer.parseInt(decimalProps
									.getProperty("item_total_reim_amount"))));
			jTextFieldResults[8].setText(DoubleUtil
					.roundWithPrecisionDoubleFormatted(
							(totValue + doubleAmount),
							Integer.parseInt(decimalProps
									.getProperty("item_total_reim_amount"))));
			((GenerateInvoice321Window) this.getTopLevelAncestor())
					.showMessage(
							Constants.LANG.getString("OKMessageReimAdded"),
							Constants.OK_MSG_COLOR);
		} else
			((GenerateInvoice321Window) this.getTopLevelAncestor())
					.showMessage(
	...
	private void delReim(java.awt.event.ActionEvent evt) {
		...
		if (row >= 0) {
			...
			String strLineAmount = ((NoEdiTableModel) jTableGlobReim.getModel())
			...
			jTextFieldTotalReim.setText(DoubleUtil
					.roundWithPrecisionDoubleFormatted((totValue - lineAmount),
							Integer.parseInt(decimalProps
									.getProperty("item_total_reim_amount"))));
			jTextFieldResults[8].setText(DoubleUtil
					.roundWithPrecisionDoubleFormatted((totValue - lineAmount),
			...
			((GenerateInvoice321Window) this.getTopLevelAncestor())
					.showMessage(
							Constants.LANG.getString("OKMessageReimDeleted"),
							Constants.OK_MSG_COLOR);
		} else
			((GenerateInvoice321Window) this.getTopLevelAncestor())
					.showMessage(
	...
	private void addAttachment(java.awt.event.ActionEvent evt) {
		...
		for (int i = 0; i < dtm.getRowCount(); ++i) {
			...
			if (dtm.getValueAt(i, 3) != null
					&& Constants.LANG.getString("Yes").equals(
		...
		InputAttachmentDialog iad = InputAttachmentDialog.getInstance(
				(JFrame) jPanelGlbDiscounts.getTopLevelAncestor(), prevDesc,
				true, true, includedLength);
		iad.setVisible(true);
		...
		if (format != null && !"".equals(format) && document != null
				&& !"".equals(document)) {// If not, it was canceled
			((NoEdiTableModel) jTableGlobAttc.getModel()).addRow(new Object[] {
					format, document, description, includingXML });
			((GenerateInvoice321Window) this.getTopLevelAncestor())
					.showMessage(
							Constants.LANG.getString("OKMessageAttachAdded"),
							Constants.OK_MSG_COLOR);
		} else
			((GenerateInvoice321Window) this.getTopLevelAncestor())
					.showMessage(
	...
	private void delAttachment(java.awt.event.ActionEvent evt) {
		...
		if (row >= 0) {
			...
			((GenerateInvoice321Window) this.getTopLevelAncestor())
					.showMessage(
							Constants.LANG.getString("OKMesaggeAttachDeleted"),
							Constants.OK_MSG_COLOR);
		} else
			((GenerateInvoice321Window) this.getTopLevelAncestor())
					.showMessage(
	...
	private void addItem(java.awt.event.ActionEvent evt,
			InvoiceLineType lineToEdit, int lineNumber) {
		Invoice321DetailDialog idd = null;
		if (jTableItems.getModel().getRowCount() == 0)
			idd = new Invoice321DetailDialog(
					(JFrame) jPanelGlbDiscounts.getTopLevelAncestor(), true,
					null, false);
		else
			idd = new Invoice321DetailDialog(
					(JFrame) jPanelGlbDiscounts.getTopLevelAncestor(),
					true,
					((GenerateInvoice321Window) this.getTopLevelAncestor()).specialTB,
					setVisualizationOnly);
		if (lineToEdit != null)
		...
		// Los campos de ob son DESCRIPTION QUANTITY
		// UNIT_OF_MEASURE,TOTAL_COST,POS_TAX_OUT,POS_TAX_WITH,ITEM_TOTAL_COST
		Object[] ob = new Object[] { "", "", "", "", "", "", "", "" };
		if (ilt.getItemDescription() != null
				&& !ilt.getItemDescription().trim().equals("")
				&& Double.valueOf(ilt.getQuantity().replace(",",".")) != 0.0 && !"".equals(ilt.getQuantity())) { // If
																				// not,
																				// it
																				// was
																				// canceled
																				// by
																				// user
			boolean modificandoLinea;
			// EDT 02/12/2015 Si lineNumber es >= 0 estamos editando una linea,
			// por lo que no se le tiene que cambiar el id
			if (lineNumber >= 0) {
				ob[Constants.POS_ITEM_LINENUMBER_GRID] = lineNumber + 1;
				modificandoLinea = true;
			} else { // EDT 02/12/2015 estamos añadiendo una linea
				ob[Constants.POS_ITEM_LINENUMBER_GRID] = jTableItems.getModel()
						.getRowCount() + 1;
				modificandoLinea = false;
			}
			ob[Constants.POS_ITEM_DESCRIPTION_GRID] = ilt.getItemDescription();
			// EDT 18/04/2016
//			ob[Constants.POS_QUANTITY_GRID] = DoubleUtil.formatDecimal(
//					ilt.getQuantity(), -1);
			ob[Constants.POS_QUANTITY_GRID] = StringUtil.disableScientificNotation(ilt.getQuantity(), -1);
//			
			if (ilt.getUnitOfMeasure() != null
					&& !ilt.getUnitOfMeasure().trim().equals("")) {
				String[] unitsOfMeasure = Constants.APP_PROP.getProperty(
						"unitsOfMeasure" + codeVersion).split(";");
				ob[Constants.POS_UNIT_OF_MEASURE_GRID] = Constants.LANG
						.getString(unitsOfMeasure[Integer.parseInt(ilt
								.getUnitOfMeasure()) - 1]);
			}
//			EDT
//			ob[Constants.POS_TOTAL_COST_GRID] = DoubleUtil.formatDecimal(ilt
//					.getTotalCost(), Integer.parseInt(decimalProps
//					.getProperty("item_total_cost")));
			ob[Constants.POS_TOTAL_COST_GRID] = StringUtil.disableScientificNotation(ilt
					.getTotalCost(), Integer.parseInt(decimalProps
					.getProperty("item_total_cost")));
			int numTaxes = 0;
			if (((es.mityc.appfacturae.facturae321.InvoiceLineType) ilt)
					.getTaxesOutputs() != null
					&& ((es.mityc.appfacturae.facturae321.InvoiceLineType) ilt)
							.getTaxesOutputs().getTax() != null)
				numTaxes = ((es.mityc.appfacturae.facturae321.InvoiceLineType) ilt)
						.getTaxesOutputs().getTax().size();
			String typeTax = "";
			boolean different = false;
			double amountTax = 0.00;
			for (int i = 0; i < numTaxes; i++) {
				Tax t = ((es.mityc.appfacturae.facturae321.InvoiceLineType) ilt)
						.getTaxesOutputs().getTax().get(i);
				if (!different) {
					if (typeTax.equals(""))
				...
				if (t.getSpecialTaxAmount() != null
						&& Double.valueOf(t.getSpecialTaxAmount().getTotalAmount().replace(",",".")) > 0.00) {
					amountTax = Double.valueOf(amountTax
							+ t.getSpecialTaxAmount().getTotalAmount().replace(",","."));
					// Special TaxableBase is Used
					jLabelTOTMessage.setText(Constants.LANG
							.getString("MessageTaxableBaseUsed"));
					jLabelTOTAsterisk.setText("*");
					((GenerateInvoice321Window) this.getTopLevelAncestor()).specialTB = true;
				} else
					amountTax = amountTax + Double.valueOf(t.getTaxAmount().getTotalAmount().replace(",","."));

				if (t.getEquivalenceSurchargeAmount() != null
						&& Double.valueOf(t.getEquivalenceSurchargeAmount().getTotalAmount().replace(",",".")) != 0)
					amountTax = amountTax
							+ Double.valueOf(t.getEquivalenceSurchargeAmount()
									.getTotalAmount().replace(",","."));
			}
			if (numTaxes > 0) {
				if (!"".equals(typeTax)
						&& !Constants.LANG.getString("DifferentValues").equals(
								typeTax)) {
					typeTax = Constants.LANG
							.getString(
									Constants.APP_PROP.getProperty(
											"taxesType" + codeVersion).split(
											";")[Integer.valueOf(typeTax) - 1])
							.split(" ")[0];
				}
				ob[Constants.POS_TAX_OUT_GRID] = typeTax
						+ " - "
						+ DoubleUtil.roundWithPrecisionDoubleFormatted(
								amountTax, Integer.parseInt(decimalProps
										.getProperty("item_tax_Out")));
			} else
				ob[Constants.POS_TAX_OUT_GRID] = Constants.LANG
						.getString(Constants.APP_PROP.getProperty(
								"specialTaxableEvent").split(";")[Integer
								.parseInt(((es.mityc.appfacturae.facturae321.InvoiceLineType) ilt)
										.getSpecialTaxableEvent()
			...
			for (int i = 0; i < numTaxesW; i++) {
				...
				amountTaxW = amountTaxW + Double.valueOf(t.getTaxAmount().getTotalAmount().replace(",","."));
			}
				...
				if (!"".equals(typeTaxW)
					...
					typeTaxW = Constants.LANG
							.getString(
									Constants.APP_PROP.getProperty(
											"taxesType" + codeVersion).split(
											";")[Integer.valueOf(typeTaxW) - 1])
							.split(" ")[0];
				}
				ob[Constants.POS_TAX_WITH_GRID] = typeTaxW
						+ " - "
						+ DoubleUtil.roundWithPrecisionDoubleFormatted(
								amountTaxW, Integer.parseInt(decimalProps
										.getProperty("item_tax_With")));
			} else
				ob[Constants.POS_TAX_WITH_GRID] = "";

			Double itemGrossCost = DoubleUtil.roundWithPrecisionDouble(Double.valueOf(ilt
					.getGrossAmount().replace(",",".")), Integer.parseInt(decimalProps
					.getProperty("item_gross_cost")));
			Double doubleObj = (Double.valueOf(itemGrossCost.toString()) + amountTax)
					- amountTaxW;
			doubleObj = DoubleUtil.roundWithPrecisionDouble(doubleObj, Integer
					.parseInt(decimalProps.getProperty("item_total_cost")));
//			EDT
//			ob[Constants.POS_ITEM_TOTAL_COST_GRID] = DoubleUtil.formatDecimal(
//					doubleObj, Integer.parseInt(decimalProps
//							.getProperty("item_total_cost")));
			ob[Constants.POS_ITEM_TOTAL_COST_GRID] = StringUtil.disableScientificNotation(
					doubleObj, Integer.parseInt(decimalProps
							.getProperty("item_total_cost")));

			if (lineToEdit != null) {
				delItem(evt, lineNumber, modificandoLinea);
				items.getInvoiceLine().add(lineNumber, ilt);
				((NoEdiTableModel) jTableItems.getModel()).insertRow(
						lineNumber, ob);
			} else {
			...
			if (jTextFieldResults[0].getText() != null
					&& !jTextFieldResults[0].getText().trim().equals(""))
				jTextFieldResults[0].setText(DoubleUtil
						.roundWithPrecisionDoubleFormatted((DoubleUtil
								.getDoubleFromPrettyNumber(jTextFieldResults[0]
										.getText()) + Double.valueOf(ilt.getGrossAmount().replace(",","."))),
								Integer.parseInt(decimalProps
										.getProperty("total_gross_amount"))));
			else
				jTextFieldResults[0].setText(DoubleUtil
						.roundWithPrecisionDoubleFormatted(
								Double.valueOf(ilt.getGrossAmount().replace(",",".")),
								Integer.parseInt(decimalProps
										.getProperty("total_gross_amount"))));

			if (jTextFieldResults[4].getText() != null
					&& !jTextFieldResults[4].getText().trim().equals(""))
				jTextFieldResults[4].setText(DoubleUtil
						.roundWithPrecisionDoubleFormatted((DoubleUtil
								.getDoubleFromPrettyNumber(jTextFieldResults[4]
										.getText()) + amountTax), Integer
								.parseInt(decimalProps
										.getProperty("item_tax_Out"))));
			else
				jTextFieldResults[4].setText(DoubleUtil
						.roundWithPrecisionDoubleFormatted(amountTax, Integer
								.parseInt(decimalProps
										.getProperty("item_tax_Out"))));

			if (jTextFieldResults[5].getText() != null
					&& !jTextFieldResults[5].getText().trim().equals(""))
				jTextFieldResults[5].setText(DoubleUtil
						.roundWithPrecisionDoubleFormatted((DoubleUtil
								.getDoubleFromPrettyNumber(jTextFieldResults[5]
										.getText()) + amountTaxW), Integer
								.parseInt(decimalProps
										.getProperty("item_tax_With"))));
			else
				jTextFieldResults[5].setText(DoubleUtil
						.roundWithPrecisionDoubleFormatted(amountTaxW, Integer
								.parseInt(decimalProps
										.getProperty("item_tax_With"))));
			tibChanged();
			recalculateTotals();
	...
	// EDT las antiguas llamadas a DelItem invocan a este metodo ahora, que al pasar un false a la nueva
	// no verán modificado su comportamiento
	private void delItem(java.awt.event.ActionEvent evt, int lineNumber) {
		delItem(evt, lineNumber, false);
	}
	...
	private void delItem(java.awt.event.ActionEvent evt, int lineNumber,
		...
		if (row >= 0) {
			...
			for (int i = 0; i < numTaxes; i++) {
				...
				if (t.getSpecialTaxAmount() != null
						&& Double.valueOf(t.getSpecialTaxAmount().getTotalAmount().replace(",",".")) > 0.00) {
					// Special TaxableBase is used. If its only one, it will be
					// deleted --> No Special Taxable Base.
			...
			jTextFieldResults[0].setText(DoubleUtil
					.roundWithPrecisionDoubleFormatted((DoubleUtil
							...
							Integer.parseInt(decimalProps
									.getProperty("item_gross_cost"))));
			Object amountTaxObject = ((NoEdiTableModel) jTableItems.getModel())
					.getValueAt(row, Constants.POS_TAX_OUT_GRID);
			if (amountTaxObject != null
					&& !"".equals(amountTaxObject.toString())) {
				...
				if (amountTaxArray[amountTaxArray.length - 1] != null
					...
					double amountTax = DoubleUtil
							.getDoubleFromPrettyNumber(amountTaxArray[amountTaxArray.length - 1]);
					jTextFieldResults[4]
							.setText(DoubleUtil.roundWithPrecisionDoubleFormatted(
									(DoubleUtil
											.getDoubleFromPrettyNumber(jTextFieldResults[4]
			...
			Object amountTaxWObject = ((NoEdiTableModel) jTableItems.getModel())
					.getValueAt(row, Constants.POS_TAX_WITH_GRID);
			if (amountTaxWObject != null
					&& !"".equals(amountTaxWObject.toString())) {
				...
				if (amountTaxWArray[amountTaxWArray.length - 1] != null
					...
					double amountTaxW = DoubleUtil
							.getDoubleFromPrettyNumber(amountTaxWArray[amountTaxWArray.length - 1]);
					jTextFieldResults[5]
							.setText(DoubleUtil.roundWithPrecisionDoubleFormatted(
									(DoubleUtil
											.getDoubleFromPrettyNumber(jTextFieldResults[5]
			...
			// EDT 02/12/2015 si estamos modificando una linea no es necesario recalcular los IDs
			if (!modificandoLinea) {
				recalculateLineNumbers(row);
			}
			tibChanged();
			recalculateTotals();
		} else
	...
	private void recalculateLineNumbers(int rowInit) {
		for (int i = rowInit; i < jTableItems.getModel().getRowCount(); i++) {
			((NoEdiTableModel) jTableItems.getModel()).setValueAt(Integer
					.parseInt(((NoEdiTableModel) jTableItems.getModel())
							.getValueAt(i, 0).toString()) - 1, i, 0);
		}
	}
	...
	private void addCharge(java.awt.event.ActionEvent evt) {
		...
		if (tibString != null && !"".equals(tibString)) {
			try {
				...
				logger.error("An number format exception occurred while a charge was being added: "
						+ e.getMessage());
				((GenerateInvoice321Window) this.getTopLevelAncestor())
						.showMessage(Constants.LANG
								.getString("NOOKParamNumericFormat"),
		...
		if (tib == null || "".equals(tib)) {
			((GenerateInvoice321Window) this.getTopLevelAncestor())
					.showMessage(
							"Debe facturar algun concepto antes de aplicar cargos",
							Constants.ERROR_MSG_COLOR);
			return;
		}
		InputChargeDialog igd = new InputChargeDialog(
				(JFrame) jPanelGlbDiscounts.getTopLevelAncestor(), true, tib,
		...
		if (item != null && !"".equals(item)) {// En caso contrario, se cancelo
			...
			Double totValue = Double.valueOf(strTotCharge);
			Double doubleAmount = Double.valueOf(amount);
			jTextFieldTotalChar.setText(DoubleUtil
					.roundWithPrecisionDoubleFormatted(totValue + doubleAmount,
							Integer.parseInt(decimalProps
									.getProperty("item_charge"))));
			jTextFieldResults[2].setText(jTextFieldTotalChar.getText());
			recalculateTotals();
			((GenerateInvoice321Window) this.getTopLevelAncestor())
					.showMessage(
							Constants.LANG.getString("OKMessageChargeAdded"),
							Constants.OK_MSG_COLOR);
		} else
			((GenerateInvoice321Window) this.getTopLevelAncestor())
					.showMessage(
	...
	private void delCharge(java.awt.event.ActionEvent evt) {
		...
		if (row >= 0) {
			String strTotCharge = jTextFieldTotalChar.getText();
			String strLineAmount = ((NoEdiTableModel) jTableGlobChar.getModel())
			...
			try {
				...
				logger.error("An number format exception occurred while a charge was being deleted: "
						+ e.getMessage());
				((GenerateInvoice321Window) this.getTopLevelAncestor())
						.showMessage(Constants.LANG
								.getString("NOOKParamNumericFormat"),
			...
			jTextFieldTotalChar
					.setText(DoubleUtil.roundWithPrecisionDoubleFormatted(
							totValue, Integer.parseInt(decimalProps
									.getProperty("item_charge"))));
			...
			((GenerateInvoice321Window) this.getTopLevelAncestor())
					.showMessage(
							Constants.LANG.getString("OKMessageChargeDeleted"),
							Constants.OK_MSG_COLOR);
		} else
			((GenerateInvoice321Window) this.getTopLevelAncestor())
					.showMessage(
	...
	public void tibChanged() {
		Double tib = DoubleUtil.getDoubleFromPrettyNumber(jTextFieldResults[0]
				.getText());
		String value = null;
		Double d = 0.0;
		...
		for (int i = 0; i < modelDisc.getRowCount(); ++i) {
			...
			if (value != null && !"".equals(value.trim())) {
				...
				d = DoubleUtil.roundWithPrecisionDouble(d, Integer
						.parseInt(decimalProps.getProperty("item_discount")));
				totDis = totDis + d;
				modelDisc.setValueAt(d, i, 2);
			} else {
				d = DoubleUtil.getDoubleFromPrettyNumber(modelDisc.getValueAt(
						i, 2).toString());
				totDis = totDis + d;
			}
		}
		jTextFieldTotalDisc.setText(DoubleUtil
				.roundWithPrecisionDoubleFormatted(totDis, Integer
						.parseInt(decimalProps.getProperty("item_discount"))));
		jTextFieldResults[1].setText(DoubleUtil
				.roundWithPrecisionDoubleFormatted(totDis, Integer
						.parseInt(decimalProps.getProperty("item_discount"))));
		...
		for (int i = 0; i < modelChar.getRowCount(); ++i) {
			...
			if (value != null && !"".equals(value.trim())) {
				...
				d = (d * tib) / 100;
				d = DoubleUtil.roundWithPrecisionDouble(d, Integer
						.parseInt(decimalProps.getProperty("item_charge")));
				totChar = totChar + d;
				modelChar.setValueAt(d, i, 2);
			} else {
				d = DoubleUtil.getDoubleFromPrettyNumber(modelChar.getValueAt(
						i, 2).toString());
				totChar = totChar + d;
			}
		}
		jTextFieldTotalChar.setText(DoubleUtil
				.roundWithPrecisionDoubleFormatted(totChar, Integer
						.parseInt(decimalProps.getProperty("item_charge"))));
		jTextFieldResults[2].setText(DoubleUtil
				.roundWithPrecisionDoubleFormatted(totChar, Integer
						.parseInt(decimalProps.getProperty("item_charge"))));
		...
		if (value != null && !"".equals(value.trim())) {
			...
			d = (d * tib) / 100;
			jTextFieldWithAmou.setText(DoubleUtil
					.roundWithPrecisionDoubleFormatted(d,
							Integer.parseInt(decimalProps
									.getProperty("item_tax_With"))));
			jTextFieldResults[7].setText(DoubleUtil
					.roundWithPrecisionDoubleFormatted(d,
	...
	public void recalculateTotals() {
		...
		if (jTextFieldResults[0].getText() != null
				&& !jTextFieldResults[0].getText().trim().equals(""))
			grossAmount = DoubleUtil.roundWithPrecisionDouble(DoubleUtil
					.getDoubleFromPrettyNumber(jTextFieldResults[0].getText()),
					Integer.parseInt(decimalProps
							.getProperty("total_gross_amount")));
		double discountAmount = 0.0;
		if (jTextFieldTotalDisc.getText() != null
				&& !jTextFieldTotalDisc.getText().trim().equals(""))
			discountAmount = DoubleUtil
					.getDoubleFromPrettyNumber(jTextFieldTotalDisc.getText());
//		EDT
//		jTextFieldResults[1].setText(DoubleUtil.formatDecimal(discountAmount,
//				Integer.parseInt(decimalProps.getProperty("discounts_total"))));
		jTextFieldResults[1].setText(StringUtil.disableScientificNotation(discountAmount,
				Integer.parseInt(decimalProps.getProperty("discounts_total"))));
//		
		double chargeAmount = 0.0;
		if (jTextFieldTotalChar.getText() != null
				&& !jTextFieldTotalChar.getText().trim().equals(""))
			chargeAmount = DoubleUtil
					.getDoubleFromPrettyNumber(jTextFieldTotalChar.getText());
//		EDT
//		jTextFieldResults[2].setText(DoubleUtil.formatDecimal(chargeAmount,
//				Integer.parseInt(decimalProps.getProperty("charges_total"))));
		jTextFieldResults[2].setText(StringUtil.disableScientificNotation(chargeAmount,
				Integer.parseInt(decimalProps.getProperty("charges_total"))));
		// BI
		Double biAmount = DoubleUtil
				.roundWithPrecisionDouble(grossAmount - discountAmount
						+ chargeAmount, Integer.parseInt(decimalProps
						.getProperty("item_taxable_base")));
//		EDT
//		jTextFieldResults[3]
//				.setText(DoubleUtil.formatDecimal(biAmount,
//						Integer.parseInt(decimalProps
//								.getProperty("item_taxable_base"))));
		jTextFieldResults[3]
				.setText(StringUtil.disableScientificNotation(biAmount,
						Integer.parseInt(decimalProps
								.getProperty("item_taxable_base"))));
		...
		// Se recorren las lÃ­neas para obtener la nueva base imponible y los
		// nuevos impuestos
		...
		for (int i = 0; i < itemsSize; i++) {
			// Importe bruto de la lÃ­nea actual
			gross2Amount = Double.valueOf(items.getInvoiceLine().get(i).getGrossAmount().replace(",","."));
			// bi2Amount = gross2Amount - discountsTotal + chargesTotal;
			bi2Amount = gross2Amount
					- (discountsTotalRate / 100.00 * gross2Amount)
					+ (chargesTotalRate / 100.00 * gross2Amount);
			// Se recorren los impuestos repercutidos
			int taxesOutputsSize = ((es.mityc.appfacturae.facturae321.InvoiceLineType) items
					.getInvoiceLine().get(i)).getTaxesOutputs().getTax().size();
			for (int j = 0; j < taxesOutputsSize; j++) {
				es.mityc.appfacturae.facturae321.InvoiceLineType.TaxesOutputs.Tax tax = ((es.mityc.appfacturae.facturae321.InvoiceLineType) items
						.getInvoiceLine().get(i)).getTaxesOutputs().getTax()
						.get(j);

				// Se coge la cuota del impuesto.
				double cuota = Double.valueOf(tax.getTaxAmount().getTotalAmount().replace(",","."));

				// Si hay cuota imponible especial se usarÃ¡ la cuota especial
				// en lugar de la cuota del impuesto
				if (tax.getSpecialTaxAmount() != null
						&& Double.valueOf(tax.getSpecialTaxAmount().getTotalAmount().replace(",",".")) != 0)
					cuota = Double.valueOf(tax.getSpecialTaxAmount().getTotalAmount().replace(",","."));

				// En ambos casos se suma el recargo de equivalencia
				if (tax.getEquivalenceSurchargeAmount() != null)
					cuota += Double.valueOf(tax.getEquivalenceSurchargeAmount()
							.getTotalAmount().replace(",","."));

				// Calculamos el porcentaje de impuestos
				taxAmountList.add(cuota - (discountsTotalRate / 100.00 * cuota)
						+ (chargesTotalRate / 100.00 * cuota));
			}
			// Se recorren los impuestos retenidos
			int taxesWithheldSize = 0;
			if (items.getInvoiceLine().get(i).getTaxesWithheld() != null
					&& items.getInvoiceLine().get(i).getTaxesWithheld()
							.getTax() != null)
				taxesWithheldSize = items.getInvoiceLine().get(i)
						.getTaxesWithheld().getTax().size();
			for (int j = 0; j < taxesWithheldSize; j++) {
				es.mityc.appfacturae.facturae.TaxType tax = items
						.getInvoiceLine().get(i).getTaxesWithheld().getTax()
						.get(j);
		...
		for (int i = 0; i < taxAmountList.size(); i++) {
			impuestoActual = DoubleUtil.roundWithPrecisionDouble(taxAmountList
					.get(i), Integer.parseInt(decimalProps
					.getProperty("item_taxable_base")));

			outputsAmount += DoubleUtil.roundWithPrecisionDouble(
					impuestoActual, Integer.parseInt(decimalProps
							.getProperty("item_taxable_base")));
		}
		...
		while (it.hasNext()) {
			...
			value = DoubleUtil.getDoubleFromPrettyNumber(e.getValue()
					.toString());
			value = DoubleUtil.roundWithPrecisionDouble(value, Integer
					.parseInt(decimalProps.getProperty("item_taxable_base")));
			withheldAmount += DoubleUtil.roundWithPrecisionDouble(
					DoubleUtil.getDoubleFromPrettyNumber(e.getKey().toString()
							.split(" - ")[1].toString())
							/ 100 * value, Integer.parseInt(decimalProps
							.getProperty("item_taxable_base")));
		}

		outputsAmount = DoubleUtil.roundWithPrecisionDouble(outputsAmount,
				Integer.parseInt(decimalProps.getProperty("item_tax_Out")));
		withheldAmount = DoubleUtil.roundWithPrecisionDouble(withheldAmount,
				Integer.parseInt(decimalProps.getProperty("item_tax_With")));
//		EDT
//		jTextFieldResults[4].setText(DoubleUtil.formatDecimal(outputsAmount,
//				Integer.parseInt(decimalProps.getProperty("item_tax_Out"))));
		jTextFieldResults[4].setText(StringUtil.disableScientificNotation(outputsAmount,
				Integer.parseInt(decimalProps.getProperty("item_tax_Out"))));
//		EDT
//		jTextFieldResults[5].setText(DoubleUtil.formatDecimal(withheldAmount,
//				Integer.parseInt(decimalProps.getProperty("item_tax_With"))));
		jTextFieldResults[5].setText(StringUtil.disableScientificNotation(withheldAmount,
				Integer.parseInt(decimalProps.getProperty("item_tax_With"))));

		double totalAmount = biAmount + outputsAmount - withheldAmount;
		try {
			totalAmount = DoubleUtil.roundWithPrecisionDouble(totalAmount,
					Integer.parseInt(decimalProps
							.getProperty("item_total_cost")));
//			EDT
//			jTextFieldResults[6].setText(DoubleUtil.formatDecimal(totalAmount,
//					Integer.parseInt(decimalProps
//							.getProperty("item_total_cost"))));
			jTextFieldResults[6].setText(StringUtil.disableScientificNotation(totalAmount,
					Integer.parseInt(decimalProps
							.getProperty("item_total_cost"))));
		} catch (NumberFormatException nfe) {
		...
		if (jTextFieldWithPerc.getText() != null
				&& !"".equals(jTextFieldWithPerc.getText().trim())) {
			jTextFieldWithAmou
					.setText(DoubleUtil.roundWithPrecisionDoubleFormatted(
							(DoubleUtil
									.getDoubleFromPrettyNumber(jTextFieldResults[6]
											.getText())
									* DoubleUtil
											.getDoubleFromPrettyNumber(jTextFieldWithPerc
													.getText()) / 100.0),
							Integer.parseInt(decimalProps
									.getProperty("global_withheld"))));
		}
		if (jTextFieldWithAmou.getText() != null
				&& !"".equals(jTextFieldWithAmou.getText().trim())) {
			jTextFieldResults[7].setText(jTextFieldWithAmou.getText());
		}

		double glbwithheldAmount = 0.0;
		if (jTextFieldResults[7].getText() != null
				&& !jTextFieldResults[7].getText().trim().equals(""))
			glbwithheldAmount = DoubleUtil
					.getDoubleFromPrettyNumber(jTextFieldResults[7].getText());
		double reimbursableAmount = 0.0;
		if (jTextFieldResults[8].getText() != null
				&& !jTextFieldResults[8].getText().trim().equals(""))
			reimbursableAmount = DoubleUtil
					.getDoubleFromPrettyNumber(jTextFieldResults[8].getText());
		double financialAmount = 0.0;
		if (jTextFieldResults[9].getText() != null
				&& !jTextFieldResults[9].getText().trim().equals("")) {
			try {
				financialAmount = DoubleUtil
						.getDoubleFromPrettyNumber(jTextFieldResults[9]
								.getText());
			} catch (NumberFormatException nfe) {
				financialAmount = 0.0;
		...
		double executableAmount = totalAmount - glbwithheldAmount
				+ reimbursableAmount + financialAmount;
		try {
			jTextFieldResults[10].setText(DoubleUtil
					.roundWithPrecisionDoubleFormatted(executableAmount,
							Integer.parseInt(decimalProps
									.getProperty("item_total_cost"))));
		} catch (NumberFormatException nfe) {
	...
	public boolean validateInstallment() {
		...
		boolean requiredGeneralFilled = (jTextFieldPaymentAmo.getText() != null && !""
				.equals(jTextFieldPaymentAmo.getText().trim()))
				|| (jComboBoxPayMean.getSelectedItem() != null && !""
						.equals(jComboBoxPayMean.getSelectedItem().toString()
								.trim()));
		boolean generalFilled = requiredGeneralFilled
				|| (jTextFieldPaymentRefIssuer.getText() != null && !""
						.equals(jTextFieldPaymentRefIssuer.getText().trim()))
				|| (jTextFieldPaymentRefReceiver.getText() != null && !""
						.equals(jTextFieldPaymentRefReceiver.getText().trim()));

		boolean requiredDebitedFilled = (jTextFieldPayDIBAN.getText() != null && !""
				.equals(jTextFieldPayDIBAN.getText().trim()));
		boolean debitedAddressFilled = (jTextFieldPayDAddr.getText() != null && !""
				.equals(jTextFieldPayDAddr.getText().trim()))
				|| (jTextFieldPayDCity.getText() != null && !""
						.equals(jTextFieldPayDCity.getText().trim()))
				|| (jTextFieldPayDPCod.getText() != null && !""
						.equals(jTextFieldPayDPCod.getText().trim()))
				|| (jTextFieldPayDProv.getText() != null && !""
						.equals(jTextFieldPayDProv.getText().trim()))
				|| (jComboBoxPayDCoun.getSelectedItem() != null && !""
						.equals(jComboBoxPayDCoun.getSelectedItem().toString()
								.trim()));
		boolean debitedFilled = requiredDebitedFilled
				|| (jTextFieldPayDBank.getText() != null && !""
						.equals(jTextFieldPayDBank.getText().trim()))
				|| (jTextFieldPayDBran.getText() != null && !""
						.equals(jTextFieldPayDBran.getText().trim()))
				|| debitedAddressFilled;

		boolean requiredCreditedFilled = (jTextFieldPayCIBAN.getText() != null && !""
				.equals(jTextFieldPayCIBAN.getText().trim()));
		boolean creditedAddressFilled = (jTextFieldPayCAddr.getText() != null && !""
				.equals(jTextFieldPayCAddr.getText().trim()))
				|| (jTextFieldPayCCity.getText() != null && !""
						.equals(jTextFieldPayCCity.getText().trim()))
				|| (jTextFieldPayCPCod.getText() != null && !""
						.equals(jTextFieldPayCPCod.getText().trim()))
				|| (jTextFieldPayCProv.getText() != null && !""
						.equals(jTextFieldPayCProv.getText().trim()))
				|| (jComboBoxPayCCoun.getSelectedItem() != null && !""
						.equals(jComboBoxPayCCoun.getSelectedItem().toString()
								.trim()));
		boolean creditedFilled = requiredCreditedFilled
				|| (jTextFieldPayCBank.getText() != null && !""
						.equals(jTextFieldPayCBank.getText().trim()))
				|| (jTextFieldPayCBran.getText() != null && !""
						.equals(jTextFieldPayCBran.getText().trim()))
				|| creditedAddressFilled;

		if (!generalFilled && !debitedFilled && !creditedFilled) {
			// OK The Installment is empty --> That's correct because the
			// installment is optional.
			return true;
		} else {
			if ((jTextFieldPaymentAmo.getText() == null || ""
					.equals(jTextFieldPaymentAmo.getText().trim()))
					|| (jComboBoxPayMean.getSelectedItem() == null || ""
							.equals(jComboBoxPayMean.getSelectedItem()
									.toString().trim()))) {
				...
				if (debitedFilled) {
					if (!requiredDebitedFilled) {
						// NO OK The account to be debited is filled but not its
						// required attributes.
						jTextFieldPayDIBAN
								.setBackground(Constants.BKG_ERROR_COLOR);
						...
						if (debitedAddressFilled) {
							if (!((jTextFieldPayDAddr.getText() != null && !""
									.equals(jTextFieldPayDAddr.getText().trim()))
									&& (jTextFieldPayDCity.getText() != null && !""
											.equals(jTextFieldPayDCity
													.getText().trim()))
									&& (((jTextFieldPayDPCod.getText() != null && !""
											.equals(jTextFieldPayDPCod
													.getText().trim()))) || (jComboBoxPayDCoun
											.getSelectedItem() != null && Constants.SPAINCODE31 != Integer
											.parseInt(((ComboOption) jComboBoxPayDCoun
													.getSelectedItem())
													.getValue())))
									&& (jTextFieldPayDProv.getText() != null && !""
											.equals(jTextFieldPayDProv
													.getText().trim())) && (jComboBoxPayDCoun
									.getSelectedItem() != null && !""
									.equals(jComboBoxPayDCoun.getSelectedItem()
											.toString().trim())))) {
								// NO OK The account to be debited address is
								// filled but not all its attributes.
								if (jTextFieldPayDAddr.getText() == null
										|| "".equals(jTextFieldPayDAddr
												.getText().trim())) {
									jTextFieldPayDAddr
											.setBackground(Constants.BKG_ERROR_COLOR);
				...
				if (creditedFilled) {
					if (!requiredCreditedFilled) {
						// NO OK The account to be credited is filled but not
						...
						if (creditedAddressFilled) {
							if (!((jTextFieldPayCAddr.getText() != null && !""
									.equals(jTextFieldPayCAddr.getText().trim()))
									&& (jTextFieldPayCCity.getText() != null && !""
											.equals(jTextFieldPayCCity
													.getText().trim()))
									&& (((jTextFieldPayCPCod.getText() != null && !""
											.equals(jTextFieldPayCPCod
													.getText().trim()))) || (jComboBoxPayCCoun
											.getSelectedItem() != null && Constants.SPAINCODE31 != Integer
											.parseInt(((ComboOption) jComboBoxPayCCoun
													.getSelectedItem())
													.getValue())))
									&& (jTextFieldPayCProv.getText() != null && !""
											.equals(jTextFieldPayCProv
													.getText().trim())) && (jComboBoxPayCCoun
									.getSelectedItem() != null && !""
									.equals(jComboBoxPayCCoun.getSelectedItem()
											.toString().trim())))) {
								// NO OK The account to be credited address is
								// filled but not all its attributes.
								if (jTextFieldPayCAddr.getText() == null
										|| "".equals(jTextFieldPayCAddr
												.getText().trim())) {
									jTextFieldPayCAddr
											.setBackground(Constants.BKG_ERROR_COLOR);
									jTextFieldPayCAddr.requestFocus();
								}
								if (jTextFieldPayCCity.getText() == null
										|| "".equals(jTextFieldPayCCity
												.getText().trim())) {
									jTextFieldPayCCity
											.setBackground(Constants.BKG_ERROR_COLOR);
								...
								if (jComboBoxPayCCoun.getSelectedItem() != null
										&& Constants.SPAINCODE31 == Integer
												.valueOf(((ComboOption) jComboBoxPayCCoun
														.getSelectedItem())
														.getValue())
										&& (jTextFieldPayCPCod.getText() == null || ""
												.equals(jTextFieldPayCPCod
								...
								if (jTextFieldPayCProv.getText() == null
										|| "".equals(jTextFieldPayCProv
												.getText().trim())) {
									jTextFieldPayCProv
											.setBackground(Constants.BKG_ERROR_COLOR);
	...
	public void setJTextFieldPaymentRefIssuer(
			JTextField textFieldPaymentRefIssuer) {
	...
	public void setJTextFieldPaymentRefReceiver(
			JTextField textFieldPaymentRefReceiver) {
	...
	private DefaultComboBoxModel payments;
	private DefaultComboBoxModel accounts;
	private DefaultComboBoxModel accounts2;
}
